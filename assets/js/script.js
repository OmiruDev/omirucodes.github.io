...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btn.addEventListener('click', () => {
    ...
    btn.setAttribute('aria-expanded', left === 0 ? 'true' : 'false');
...
(() => {
  ...
  const createTrack = () => {
    const tailLength = Math.floor(8 + Math.random() * 12);
    return {
      y: Math.random() * rowCount,
      speed: 0.12 + Math.random() * 0.3,
      tailLength,
      chars: Array.from({ length: tailLength }, randomBinary)
    };
  };
  ...
  const setupCanvas = () => {
    ...
    fontSize = width < 575 ? 11 : width < 992 ? 12 : 13;
    colCount = Math.max(1, Math.floor(width / fontSize));
    rowCount = Math.max(1, Math.ceil(height / fontSize));
    tracks = Array.from({ length: colCount }, createTrack);
  };

  const drawTrail = (trailColor) => {
  ...
  const draw = (time) => {
    ...
    const trail = getCssVar('--binary-rain-trail', 'rgba(2, 12, 8, 0.14)');
    const bright = getCssVar('--binary-rain-char-bright', 'rgba(96, 255, 154, 0.92)');
    const dim = getCssVar('--binary-rain-char-dim', 'rgba(62, 255, 126, 0.56)');
    ...
    for (let col = 0; col < colCount; col++) {
      ...
      const headRow = Math.floor(track.y);

      for (let i = 0; i < track.tailLength; i++) {
        const row = headRow - i;
        if (row < 0 || row > rowCount) continue;

        if (Math.random() > 0.84) {
          track.chars[i] = randomBinary();
        }

        const y = row * fontSize;
        const fade = 1 - i / track.tailLength;
        const alpha = Math.max(0.15, fade);

        ctx.fillStyle = i === 0 ? bright : dim;
        ctx.globalAlpha = alpha;
        ctx.fillText(track.chars[i], x, y);
      }

      ctx.globalAlpha = 1;
      track.y += track.speed;

      if (track.y - track.tailLength > rowCount + 2) {
        tracks[col] = createTrack();
        tracks[col].y = -Math.random() * rowCount * 0.55;
      }
    }
  ...
  const drawStaticFrame = () => {
    const trail = getCssVar('--binary-rain-trail', 'rgba(2, 12, 8, 0.16)');
    ...
    for (let col = 0; col < colCount; col++) {
      for (let row = 0; row < rowCount; row++) {
        ...
        const x = col * fontSize;
        const y = row * fontSize;
        ctx.fillStyle = Math.random() > 0.75 ? bright : dim;
        ctx.globalAlpha = 0.62;
        ctx.fillText(randomBinary(), x, y);
      }
    }
    ctx.globalAlpha = 1;
  };
